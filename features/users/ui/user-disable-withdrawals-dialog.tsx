...
import { useDisableUserWithdrawals } from "@/features/users/usecases"

type UserDisableWithdrawalsDialogProps = {
  user: User
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function UserDisableWithdrawalsDialog({
  user,
  open,
  onOpenChange,
}: UserDisableWithdrawalsDialogProps) {
  ...
  const mutation = useDisableUserWithdrawals(user.id)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-0 p-0 sm:max-w-lg">
        <DialogHeader className="border-b px-4 py-4">
          <DialogTitle>Disable withdrawals</DialogTitle>
          ...
          <div className="overflow-y-auto px-4 py-4">
            <FieldGroup>
              <Field data-invalid={errors.reason ? true : undefined}>
                <FieldLabel htmlFor="disable-withdrawals-reason">
                  Reason
                </FieldLabel>
                <Textarea
                  id="disable-withdrawals-reason"
                  placeholder="Why should withdrawals be disabled for this user?"
                  rows={4}
